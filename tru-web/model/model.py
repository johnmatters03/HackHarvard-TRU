...
def fparse(chain, content, chunk=1000, overlap=250):
    result = ""
    counter = 0
    while counter < len(content):
        result += chain.run({'previous': result[max(0, len(result) - overlap):], 'transcript' : content[counter : min(counter+chunk, len(content))]})
        counter += chunk
    return result

def fcat(chain, content, chunk=1000):
    result = ""
    counter = 0
    while counter < len(content):
        result += chain.run({'subject' : "lynn", 'seg' : content[counter : min(len(content), counter + chunk)]})
        counter += chunk
    return result
...
def biography(src):
    ...
    t_parse = """
    You are having a conversation with your grandparent Lynn Ruth Miller, and you want to continue writing a segment of their biography.
    ...
    p_parse = PromptTemplate(
        input_variables=["previous", "transcript"],
        template=t_parse,
    )
    ...
    for query in comb.split('\n'):
        chap = c_final.run({"subject" : "Lynn", "topic" : query, "reference" : search.similarity_search(query)})
        final_bio += chap
        final_bio += "\n"
